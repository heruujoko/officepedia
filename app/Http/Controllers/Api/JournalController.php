...
class JournalController extends Controller
{
    ...
    public function group_journal($type){
        ...
        return Datatables::of($grp)->addColumn('action', function($j){
            $this->journals = MJournal::on(Auth::user()->db_name)->where('mjournalid',$j->mjournalid)->where('void',0)->get();
          return '<center><div class="button">
            ...
            foreach($this->journals as $jr){
                if($jr->mjournaldebit != 0){
                    $html_str.="<span style=\"float:left;\">".number_format($jr->mjournalcredit,$this->decimals,$this->dec_point,$this->thousands_sep)."</span><br><br>";
                } else {
                    $html_str.="<span style=\"float:right;\">".number_format($jr->mjournalcredit,$this->decimals,$this->dec_point,$this->thousands_sep)."</span><br><br>";
                }
            }
            $html_str.="</div>";
            return $html_str;
            ...
            foreach($this->journals as $jr){
                if($jr->mjournaldebit != 0){
                    $html_str.="<span style=\"float:left;\">".number_format($jr->mjournaldebit,$this->decimals,$this->dec_point,$this->thousands_sep)."</span><br><br>";
                } else {
                    $html_str.="<span style=\"float:right;\">".number_format($jr->mjournaldebit,$this->decimals,$this->dec_point,$this->thousands_sep)."</span><br><br>";
                }
            }
            $html_str.="</div>";
            return $html_str;
