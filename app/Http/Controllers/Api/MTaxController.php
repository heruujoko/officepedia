...
class MTaxController extends Controller
{
    ...
    public function index(){
      ...
      return Datatables::of($mtax)->addColumn('action', function($mtax){
          ...
          if(Auth::user()->has_role('U_tax')){
                $menus .= '<a class="btn btn-primary btn-xs dropdown-toggle fa fa-pencil" onclick="editmtax('.$mtax->id.')"> <font style="font-family: arial;">Ubah &nbsp</font></a>';
          }
          if(Auth::user()->has_role('D_tax')){
              $menus .= '<a class="btn btn-danger btn-xs dropdown-toggle fa fa-trash" onclick="popupdelete('.$mtax->id.')">
