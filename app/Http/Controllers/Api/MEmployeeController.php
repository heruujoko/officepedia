    ...
    public function index(){
      $this->iteration = 0;
          ...
          return Datatables::of($mcategory)->addColumn('action', function($memployee){
              ...
              if(Auth::user()->has_role('U_employee')){
                  $menus .= '<a class="btn btn-primary btn-xs dropdown-toggle fa fa-pencil" onclick="editmemployee('.$memployee->id.')"> <font style="font-family: arial;">Ubah &nbsp</font></a>';
              }
              if(Auth::user()->has_role('D_employee')){
                    $menus .= '<a class="btn btn-danger btn-xs dropdown-toggle fa fa-trash" onclick="popupdelete('.$memployee->id.')">
