...
class MCustomerController extends Controller
{
	public function index(){
        ...
        return Datatables::of($mcustomer)->addColumn('action', function($mcustomer){
            ...
            if(Auth::user()->has_role('U_customer')){
                $menus .= '<a class="btn btn-primary btn-xs dropdown-toggle fa fa-pencil" onclick="editmcustomer('.$mcustomer->id.')"> <font style="font-family: arial;">Ubah &nbsp</font></a>';
            }
            if(Auth::user()->has_role('D_customer')){
                $menus .= '<a class="btn btn-danger btn-xs dropdown-toggle fa fa-trash" onclick="popupdelete('.$mcustomer->id.')">
