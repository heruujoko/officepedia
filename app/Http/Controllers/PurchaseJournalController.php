...
class PurchaseJournalController extends Controller
{
    public function index(){
        if(Auth::user()->has_role('R_journal')){
            $data['active'] = 'purchasejournal';
            $data['section'] = 'Jurnal Pembelian';
            $data['config'] = MConfig::on(Auth::user()->db_name)->where('id',1)->first();
            return view('admin.purchasejournalreport',$data);
        } else {
            return redirect('/admin-nano/index');
        }
    }

    private function purchasejournal_data($request){
    ...
    public function purchasejournal_print(Request $request){
        ...
        $data['journals'] = $this->purchasejournal_data($request);
        $data['mode'] = 'Pembelian';
    ...
    public function purchasejournal_pdf(Request $request){
        $conf = MConfig::on(Auth::user()->db_name)->where('id',1)->first();
		...
		return $pdf->setPaper('a4', 'portrait')->download('Purchase Journal.pdf');
    }

    public function purchasejournal_excel(Request $request){
        $conf = MConfig::on(Auth::user()->db_name)->where('id',1)->first();
        ...
        $this->data['journals'] = $this->purchasejournal_data($request);
    ...
    public function purchasejournal_csv(Request $request){
        $conf = MConfig::on(Auth::user()->db_name)->where('id',1)->first();
